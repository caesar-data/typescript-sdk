// File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.

import { maybeFilter } from 'caesar-mcp/filtering';
import { Metadata, asTextContentResult } from 'caesar-mcp/tools/types';

import { Tool } from '@modelcontextprotocol/sdk/types.js';
import Caesar from 'caesar-data';

export const metadata: Metadata = {
  resource: 'x402.research',
  operation: 'write',
  tags: [],
  httpMethod: 'post',
  httpPath: '/x402/research',
  operationId: 'createResearchObjectX402',
};

export const tool: Tool = {
  name: 'create_x402_research',
  description:
    "When using this tool, always use the `jq_filter` parameter to reduce the response size and improve performance.\n\nOnly omit if you're sure you don't need the data.\n\nStart a new research job using x402 payment. This endpoint mints a temporary API key that is returned in the response and is billed via the x402 settlement flow instead of your Caesar API credits.\n\n# Response Schema\n```json\n{\n  $ref: '#/$defs/research_create_response',\n  $defs: {\n    research_create_response: {\n      type: 'object',\n      properties: {\n        id: {\n          type: 'string',\n          description: 'Research job identifier.'\n        },\n        status: {\n          type: 'string',\n          description: 'Current status of the research job.',\n          enum: [            'queued',\n            'searching',\n            'summarizing',\n            'analyzing',\n            'completed',\n            'failed',\n            'researching'\n          ]\n        },\n        api_key_secret: {\n          type: 'string',\n          description: 'Temporary API key secret created for this x402 request.'\n        }\n      },\n      required: [        'id',\n        'status'\n      ]\n    }\n  }\n}\n```",
  inputSchema: {
    type: 'object',
    properties: {
      query: {
        type: 'string',
        description: 'Primary research question or instruction.',
      },
      'X-PAYMENT': {
        type: 'string',
      },
      compute_units: {
        type: 'integer',
        description: 'Optional compute budget for the job. Defaults to 1.',
      },
      system_prompt: {
        type: 'string',
        description: 'Optional system prompt to steer the assistant.',
      },
      jq_filter: {
        type: 'string',
        title: 'jq Filter',
        description:
          'A jq filter to apply to the response to include certain fields. Consult the output schema in the tool description to see the fields that are available.\n\nFor example: to include only the `name` field in every object of a results array, you can provide ".results[].name".\n\nFor more information, see the [jq documentation](https://jqlang.org/manual/).',
      },
    },
    required: ['query', 'X-PAYMENT'],
  },
  annotations: {},
};

export const handler = async (client: Caesar, args: Record<string, unknown> | undefined) => {
  const { jq_filter, ...body } = args as any;
  return asTextContentResult(await maybeFilter(jq_filter, await client.x402.research.create(body)));
};

export default { metadata, tool, handler };
